...
- Containerized the refresh workflow with Docker and Azure Container Registry
- Scheduled routine refreshes through Azure Container Apps Jobs as a cost-aware batch execution path
- Validated Azure Databricks as an optional managed Spark validation path, then disabled routine scheduling for cost control
...
This is a scheduled snapshot dashboard, not a live operations monitoring system. Routine refresh is handled by Azure Container Apps Jobs, while Azure Blob Storage acts as the serving boundary for the latest exported parquet outputs. Databricks was used as an optional managed Spark validation path, but it is not part of the routine refresh path.

Weather is used as contextual external information only. The project does not infer causal weather impact.

**Tech stack:**  
PySpark · Delta-style Medallion Architecture · Docker · Azure Container Apps Jobs · Azure Container Registry · Azure Blob Storage · Azure Databricks · Azure Functions · Streamlit · Render · GitHub Actions · OpenAI API
