...
def validate(path: Path) -> None:
    ...
    assert not trailing_whitespace_lines, (
        "Trailing whitespace found on lines: " f"{trailing_whitespace_lines}"
    )
    ...
    assert (
        set(rule_ids) == expected_rule_ids
    ...
    for rule_id, rule in rules_by_id.items():
        ...
        assert not missing_fields, f"{rule_id} missing fields: {sorted(missing_fields)}"
    ...
    assert r004["trigger_logic"]["operator"] == ("baseline_minus_current_not_empty")
    assert set(r004["trigger_logic"]["filters"]["resource_type_in"]) == {"model"}
    ...
    for rule in rules:
        for evidence in rule["required_evidence"]:
            assert evidence["source"] in allowed_sources, (
                f"{rule['rule_id']} references unknown source " f"{evidence['source']}"
            )
